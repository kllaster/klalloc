...
t_body_alloc_node *get_next_node(const t_body_alloc_meta *ba_meta, const t_body_alloc_node *node)
{
	...
	if ((void *)node >= ba_meta->end)
		return NULL;
	return (t_body_alloc_node *)node;
}

static size_t get_max_node_size(const t_body_alloc_meta *ba_meta)
{
...
static void
body_alloc_split_node(t_body_alloc_node *node, size_t old_size, size_t new_size, bool first_free)
{
...
static void check_right_segment(t_body_alloc_meta *ba_meta, t_body_alloc_node *node)
{
	t_body_alloc_node *right_node;

	right_node = move_pointer_node(node, node->size + sizeof(t_body_alloc_node) * 2);
	if ((char *)right_node + sizeof(t_body_alloc_node) > (char *)ba_meta->end)
		return;
...
check_left_segment(t_body_alloc_meta *ba_meta, t_body_alloc_node *node)
{
...
void body_alloc_free(t_body_alloc_meta *ba_meta, void *ptr)
{
	...
	if ((char *)node_end + sizeof(t_body_alloc_node) > (char *)ba_meta->end)
		return;
...
static inline bool ptr_in_body_alloc_single(const t_body_alloc_meta *ba_meta, void *ptr)
{
	if (ptr >= ba_meta->start && ptr < ba_meta->end)
		return true;
	return false;
...
t_body_alloc_meta *ptr_in_body_alloc_list(const t_body_alloc_meta *ba_meta, void *ptr)
{
	while (ba_meta)
	{
		if (ptr_in_body_alloc_single(ba_meta, ptr))
			return (t_body_alloc_meta *)ba_meta;
